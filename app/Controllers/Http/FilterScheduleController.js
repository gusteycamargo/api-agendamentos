...
class FilterScheduleController {
    async index ({ request, auth, response, view }) {
        const data = request.only(['date', 'period']);
        ...
        if(auth.user.function === 'adm') {
          schedules = await Schedule.query()
            .whereRaw('date = ? and (initial between ? and ?) and (final between ? and ?)', 
                [data.date, periodHour.initial1, periodHour.initial2, periodHour.final1, periodHour.final2])
            .with('place')
          ...
          schedules = await Schedule.query().whereRaw('(requesting_user_id = ?) and date = ? and (initial between ? and ?) and (final between ? and ?)', 
                [auth.user.id, data.date, periodHour.initial1, periodHour.initial2, periodHour.final1, periodHour.final2])
            .with('place')
            .with('requesting_user')
            .with('registration_user')
            .with('equipaments')
